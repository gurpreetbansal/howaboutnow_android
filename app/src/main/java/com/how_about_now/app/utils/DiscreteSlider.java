...
public class DiscreteSlider extends View {
	...
	private void init(Context context, @Nullable AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			mTrackWidth = a.getDimension(R.styleable.DiscreteSlider_ds_trackWidth,
					DiscreteSliderUtils.convertDpToPixel(4, getContext()));
			...
			mRadius = a.getDimension(R.styleable.DiscreteSlider_ds_thumbRadius,
					DiscreteSliderUtils.convertDpToPixel(6, getContext()));
			...
			mValueLabelTextSize = a.getDimension(R.styleable.DiscreteSlider_ds_valueLabelTextSize,
					DiscreteSliderUtils.convertSpToPixel(16, context));
			...
			if (a.hasValue(R.styleable.DiscreteSlider_ds_tickMarkPatterns)) {
				...
				if (!TextUtils.isEmpty(patterns)) {
					float length = a.getDimension(R.styleable.DiscreteSlider_ds_tickMarkDashLength,
							DiscreteSliderUtils.convertDpToPixel(1, context));
			...
			mTrackWidth = DiscreteSliderUtils.convertDpToPixel(4, context);
			mTrackColor = 0xff5123da;
			mInactiveTrackColor = 0x3d5123da;

			mRadius = DiscreteSliderUtils.convertDpToPixel(6, context);
			...
			mValueLabelTextSize = DiscreteSliderUtils.convertSpToPixel(16, context);
	...
	protected void onDraw(Canvas canvas) {
		...
		float _cx = cx;
		float _cy = cy;
		float dp6 = DiscreteSliderUtils.convertDpToPixel(6, getContext());
		float ratio = mRadius / dp6;
		if (mOrientation == HORIZONTAL) {
			if (mValueLabelGravity == TOP) {
				_cy -= dp6 + DiscreteSliderUtils.convertDpToPixel(16, getContext()) + dp6 * 3;
			...
			if (mValueLabelGravity == RIGHT) {
				_cx += dp6 + DiscreteSliderUtils.convertDpToPixel(16, getContext()) + dp6 * 3;
	...
	private void generateValueLabelPath() {
		float r2 = DiscreteSliderUtils.convertDpToPixel(6, getContext()), cx2, cy2;
		...
		float dp1 = DiscreteSliderUtils.convertDpToPixel(1, getContext()), dp16 =
				DiscreteSliderUtils.convertDpToPixel(16, getContext());
